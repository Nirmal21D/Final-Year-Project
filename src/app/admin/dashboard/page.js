  ...
  useEffect(() => {
    const fetchBankAndPlanData = async () => {
      try {
        ...
        const [banksSnap, plansSnap, usersSnap, loanAppsSnap] =
          await Promise.all([
            getDocs(banksRef),
            getDocs(plansRef),
            getDocs(usersRef),
        ...
        setPendingVerifications(
          banksData.filter((bank) => !bank.isVerified && !bank.isRejected)
            .length
        );
    ...
    loanApps.forEach((doc) => {
      ...
      if (date && data.loanAmount) {
        ...
        monthlyData[monthYear] =
          (monthlyData[monthYear] || 0) + Number(data.loanAmount);
      }
    });
  ...
  const fetchBankPlanStats = async (bankId) => {
    try {
      const [investmentPlans, loanPlans] = await Promise.all([
        getDocs(
          query(
            ...
            where("createdBy", "==", bankId)
          )
        ),
        getDocs(
          query(collection(db, "loanplans"), where("createdBy", "==", bankId))
        ),
      ]);
      ...
      return {
        ...
        activeInvestmentPlans: investmentPlans.docs.filter(
          (doc) => doc.data().isVerified
        ).length,
        activeLoanPlans: loanPlans.docs.filter((doc) => doc.data().isVerified)
          .length,
      };
    } catch (error) {
              ...
              <Flex justify="space-between" align="center" mb={4}>
                ...
                <ButtonGroup>
                  <Menu>
                    ...
                    <MenuList>
                      <MenuItem onClick={() => setBankFilter("all")}>
                        All Banks
                      </MenuItem>
                      <MenuItem onClick={() => setBankFilter("pending")}>
                        Pending Verification
                      </MenuItem>
                      <MenuItem onClick={() => setBankFilter("verified")}>
                        Verified
                      </MenuItem>
                      <MenuItem onClick={() => setBankFilter("rejected")}>
                        Rejected
                      </MenuItem>
                    </MenuList>
                  </Menu>
              ...
              >
                <Table variant="simple">
                  ...
                  <Tbody>
                    {banks
                      .filter((bank) => {
                        if (bankFilter === "pending")
                          return !bank.isVerified && !bank.isRejected;
                            ...
                            <SimpleGrid columns={2} spacing={2}>
                              <Stat size="sm">
                                ...
                                <StatNumber fontSize="md">
                                  {planStats[bank.id]?.totalInvestmentPlans ||
                                    0}
                                </StatNumber>
                              </Stat>
                              <Stat size="sm">
                                ...
                                <StatNumber fontSize="md">
                                  {planStats[bank.id]?.activeInvestmentPlans ||
                                    0}
                                </StatNumber>
                              </Stat>
                            </SimpleGrid>
                          ...
                          <Td>
                            <SimpleGrid columns={2} spacing={2}>
                              <Stat size="sm">
                                <StatLabel fontSize="xs">Total</StatLabel>
                                <StatNumber fontSize="md">
                                  {planStats[bank.id]?.totalLoanPlans || 0}
                                </StatNumber>
                              </Stat>
                              <Stat size="sm">
                                <StatLabel fontSize="xs">Active</StatLabel>
                                <StatNumber fontSize="md">
                                  {planStats[bank.id]?.activeLoanPlans || 0}
                                </StatNumber>
                              </Stat>
                            </SimpleGrid>
                          </Td>
